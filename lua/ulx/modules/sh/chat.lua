...
function ulx.psay( calling_ply, target_ply, message )
	...
	if (IsValid(calling_ply)) then
		if calling_ply.ACS_LastChatTime == nil then
			calling_ply.ACS_LastChatTime = 0
		end
		if calling_ply.ACS_RepeatChatCount == nil then
			calling_ply.ACS_RepeatChatCount = 0
		end
		if calling_ply.ACS_ChatCount == nil then
			calling_ply.ACS_ChatCount = 0
		end
		if calling_ply.ACS_LastMsg == nil then
			calling_ply.ACS_LastMsg = ""
		end
		-- repeating text?
		if message == calling_ply.ACS_LastMsg then
			calling_ply.ACS_RepeatChatCount = calling_ply.ACS_RepeatChatCount + 1
		else
	...
	if calling_ply.Nick then
		game.ConsoleCommand("say AdminHide:PSAY "..calling_ply:Nick() .. ":" .. target_ply:Nick() .." - " .. message.."\n")
	else
		game.ConsoleCommand("say AdminHide:PSAY "..  target_ply:Nick() .." - " .. message.."\n")
	end
...
function ulx.asay( calling_ply, message )
	...
	if calling_ply.Nick then
		game.ConsoleCommand("say AdminHide:ASAY "..calling_ply:Nick().." - " .. message.."\n")
	else
		game.ConsoleCommand("say AdminHide:ASAY " .. message.."\n")
	end
...
asay:help( "Send a message to currently connected staff." )
